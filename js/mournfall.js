/*
 * mournfall.js — THE CITY OF MOURNFALL (an area of Kaelinu's beyond).
 *
 * A 158-room area: a city caught between worlds, in the Far-Land at the end of
 * everything, roofed by an obsidian dome that holds back the Void. Content, not
 * framework — a MUD zone file. It is populated into a shared World by the
 * orchestrator in world-data.js, which passes in the builder context (the room
 * table and the room()/link() helpers) and flushes the links once every area
 * has been built.
 *
...
(function (global) {
  ...
  MUD.buildMournfall = function buildMournfall(ctx) {
    ...
    // The undercity has teeth. Ghouls lair in the deep and hunt the dead.
    R.under_nest.add(world.mob({
      name: 'pale ghoul',
      keywords: ['ghoul', 'pale'],
      short: 'A pale ghoul crouches over gnawed bone, and turns to look at you.',
      description:
        'A wet-grey, hairless thing, all knuckle and tooth, that has eaten so ' +
        'many of Mournfall\'s dead it has begun to resemble them. Its eyes are ' +
        'lightless. Its hunger is not.',
      hostile: true,
      level: 4,
      maxHp: 34,
      stats: { str: 15, dex: 13, con: 13, int: 5 },
      damage: [3, 7],
      armor: 1,
      xp: 90,
      respawns: 60,
    }));
    R.under_warren.add(world.mob({
      name: 'bone-picker',
      keywords: ['bone-picker', 'picker', 'wretch'],
      short: 'A hunched bone-picker sifts the dark, muttering.',
      description:
        'One of the born-below, gone feral in the warrens — less a person now ' +
        'than an appetite in a person\'s shape, clutching a sharpened shin.',
      hostile: true,
      level: 2,
      maxHp: 20,
      stats: { str: 12, dex: 12, con: 11, int: 6 },
      damage: [2, 5],
      xp: 45,
      respawns: 45,
    }));

    return { area: mournfall, arrivalId: 'gate_arrival' };
  };
